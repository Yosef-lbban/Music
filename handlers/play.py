...
async def play(_, message: Message):
    ...
    if audio:
        ...
        return await message.reply_text(f"**{bn} :-** قم بالرد على المادة او الرابط اولا ♻️")

    if message.chat.id in callsmusic.pytgcalls.active_calls:
        await message.reply_text(f"**{bn} :-** تم اضافه المادة الى الدور #{await callsmusic.queues.put(message.chat.id, file_path=file_path)} 🇸🇦")
    else:
        callsmusic.pytgcalls.join_group_call(message.chat.id, file_path)
        await message.reply_text(f"**{bn} :-** تم تشغيل المادة.. 🇸🇦")
